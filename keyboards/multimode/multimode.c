...
#            define id_dynamic_keymap_get_encoder 0x14
#        endif
#        ifndef id_dynamic_keymap_set_encoder
#            define id_dynamic_keymap_set_encoder 0x15
#        endif
#    endif
...
__WEAK void via_set_device_indication(uint8_t value) {
    (void)value;
}
...
void raw_hid_receive_kb(uint8_t *data, uint8_t length) {
    ...
    if (*command_id >= GET_QMK_VERSION && *command_id <= GET_FIRMWARE_TYPE && srgb_raw_hid_rx_no_send(data, length)) {
        return;
    }
...
#ifdef SIGNALRGB_SUPPORT_ENABLE
    if (*command_id >= GET_QMK_VERSION && *command_id <= GET_FIRMWARE_TYPE) {
